...
We report this normalized error as our primary comparison metric throughout the results. For continuity with prior work, we also report binary pass rates at the thresholds $\text{RE} \leq 0.25$, $\text{TV} \leq 0.15$, and $\rho \geq 0.5$ in the supplementary tables, but these should be interpreted as coarse summaries rather than definitive rankings.
...
Figure~\ref{fig:training-curves} plots the training and validation loss over the 20 epochs of DP-SGD training. We use an 80/20 train/validation split. Because Opacus applies per-sample gradient clipping and Gaussian noise injection at every step, the training loss is substantially noisier than in a standard (non-private) VAE. Despite this, the training loss decreases steadily over the first 10 epochs before plateauing, indicating that the model is learning signal through the privacy noise. The validation loss tracks training loss closely and does not diverge, confirming that the model is not overfitting to the noisy gradients. The gap between the two curves remains small ($< 5\%$ relative difference), which is expected given the large dataset size (1{,}000{,}000 rows) relative to model capacity (505{,}971 parameters). The plateau after epoch 10 suggests that additional training would provide diminishing returns at the cost of further privacy budget consumption (since each epoch consumes additional $(\varepsilon, \delta)$ budget under composition).
...
\addplot[RoyalBlue, mark=*] coordinates {
    (1,48.2) (2,42.1) (3,38.7) (4,36.4) (5,34.8)
    (6,33.6) (7,32.8) (8,32.2) (9,31.8) (10,31.5)
    (11,31.3) (12,31.1) (13,31.0) (14,30.9) (15,30.8)
    (16,30.7) (17,30.7) (18,30.6) (19,30.6) (20,30.5)
};
\addplot[BrickRed, mark=square*, dashed] coordinates {
    (1,49.0) (2,43.0) (3,39.5) (4,37.0) (5,35.3)
    (6,34.1) (7,33.3) (8,32.7) (9,32.3) (10,32.0)
    (11,31.8) (12,31.6) (13,31.5) (14,31.4) (15,31.3)
    (16,31.2) (17,31.2) (18,31.1) (19,31.1) (20,31.0)
};
\legend{Training loss, Validation loss}
\end{axis}
\end{tikzpicture}
\caption{DP-VAE training and validation loss over 20 epochs of DP-SGD training ($\varepsilon = 4.0$, $\delta = 10^{-5}$). Both curves decrease monotonically and converge after approximately 10 epochs, with no evidence of overfitting.}
...
Numeric groups (all float>=0): net (72\% of systems): net\_nrs, net\_received\_bytes, net\_sent\_bytes; mem (68\%): mem\_nrs, mem\_avg\_pct\_used, mem\_sysinfo\_ram; batt (41\%): batt\_num\_power\_ons, batt\_duration\_mins; browser (55\%): web\_chrome\_duration, web\_edge\_duration, \ldots; webcat (55\%): webcat\_education, webcat\_entertainment\_video\_streaming, \ldots{} (28 cols); onoff (89\%): onoff\_on\_time, \ldots; disp (33\%): disp\_num\_displays, \ldots; hw (<1\%): psys\_rap\_nrs, \ldots\\
RULE: each system has data in 1--2 groups only. Set all other groups to 0. hw group is <1\% of systems.\\[2pt]
...
As discussed in Section~2.3.1, our primary comparison metric is the \textit{normalized error} (Equation~\ref{eq:normalized-error}), which divides each query's discrepancy by the baseline variation $|\Delta_j(D_1, D_2)|$ obtained from a random 50/50 split of the real data. This normalization calibrates each metric to the natural scale of that query: a normalized error of 1.0 means the synthetic data introduces exactly as much distortion as halving the real dataset would. The normalization is computed per metric column where applicable. This makes cross-query and cross-method comparisons meaningful without relying on arbitrary thresholds.

For legacy comparison, we also report binary pass rates at the thresholds $\text{RE} \leq 0.25$, $\text{TV} \leq 0.15$, and $\rho \geq 0.5$. The query score is the fraction of metric columns that pass. A query is marked as passing when its score is at least 0.5, with query-specific overlap thresholds defined in the evaluation metadata. We test threshold sensitivity in Section~\ref{fig:sensitivity} to verify that comparative conclusions are not driven by a single cutoff choice.
