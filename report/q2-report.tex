...
% \begin{footnotesize}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.38\linewidth}>{\raggedright\arraybackslash}p{0.13\linewidth}ccc}
\caption{Per-query scores for all three synthesis methods, color-coded by fidelity: \colorbox{scorehi}{\strut$\geq 0.5$} (pass), \colorbox{scoremid}{\strut$0.25$--$0.49$}, \colorbox{scorelo}{\strut$< 0.25$}, \colorbox{scorena}{\strut N/A}. Scores marked $\checkmark$ pass the 0.5 threshold.}
...
\bottomrule
\endlastfoot
\texttt{avg\_platform\_power\_c0\_freq\_temp\_by\_chassis} & Agg+Join & \cellcolor{scorelo}0.167 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{battery\_power\_on\_geographic\_summary} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}0.500\,$\checkmark$ \\
\texttt{Xeon\_network\_consumption} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 \\
\texttt{persona\_web\_cat\_usage\_analysis} & Pivot & \cellcolor{scorelo}0.065 & \cellcolor{scoremid}0.419 & \cellcolor{scorelo}0.194 \\
\texttt{pkg\_power\_by\_country} & Geo/Demo & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 \\
\texttt{most\_popular\_browser\ldots country} & Top-$k$ & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ \\
\texttt{popular\_browsers\ldots percentage} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & \cellcolor{scorehi}0.667\,$\checkmark$ \\
\texttt{ram\_utilization\_histogram} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500\,$\checkmark$ & \cellcolor{scorelo}0.000 \\
\midrule
\texttt{battery\_on\_duration\_cpu\_family\_gen} & Geo/Demo & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{display\_devices\ldots durations\_ac\_dc} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{display\_devices\_vendors\_percentage} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ \\
\texttt{mods\_blockers\_by\_osname\ldots} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 \\
\texttt{on\_off\_mods\_sleep\_summary\ldots} & Pivot & \cellcolor{scorena}--- & \cellcolor{scorehi}0.636\,$\checkmark$ & \cellcolor{scoremid}0.273 \\
\texttt{server\_exploration\_1} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.143 & \cellcolor{scoremid}0.429 \\
\texttt{top\_mods\_blocker\_types\ldots} & Agg+Join & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{top\_10\_apps\ldots focal\_time} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{top\_10\_apps\ldots system\_count} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}1.000\,$\checkmark$ \\
\texttt{top\_10\_apps\ldots total\_detections} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorehi}0.500\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ \\
\texttt{userwait\_top\_10\_wait\_processes} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{userwait\_top\_10\ldots wait\_type\_ac\_dc} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\texttt{userwait\_top\_20\ldots ac\_dc\_unknown} & Top-$k$ & \cellcolor{scorena}--- & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 \\
\end{longtable}
% \end{footnotesize}
...
\begin{axis}[
    ...
    symbolic x coords={Wide,Per-table,MST},
    xtick=data,
    ymin=0, ymax=9,
    ytick={0,2,4,6,8},
    nodes near coords,
...
\addplot[fill=RoyalBlue!50, draw=RoyalBlue] coordinates {(Wide,1) (Per-table,6) (MST,6)};
\end{axis}
\end{tikzpicture}
\end{minipage}
\hfill
...
\addplot[fill=RoyalBlue!50, draw=RoyalBlue] coordinates {(Wide,0.258) (Per-table,0.303) (MST,0.328)};
\end{axis}
\end{tikzpicture}
\end{minipage}
...
\legend{Wide DP-SGD, Per-table DP-SGD, MST}
\end{axis}
\end{tikzpicture}
