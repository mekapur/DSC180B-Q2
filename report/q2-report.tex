...
\end{equation}
For ranked result sets, we compute Spearman's rank correlation coefficient on the overlapping items. Group coverage is assessed via Jaccard similarity between real and synthetic group keys. Each metric is evaluated against its query-type threshold, then query score is computed as the fraction of per-column metrics that pass. A query is marked as passing when this score is at least 0.5. The default thresholds are relative error $\leq 0.25$, total variation $\leq 0.15$, and Spearman $\rho \geq 0.5$, with query-specific overlap thresholds defined in the evaluation metadata.
...
\noindent Jason Tran. They built the 19 reporting tables from raw telemetry data via DuckDB aggregation scripts. They designed and implemented the wide-table construction, DP-VAE architecture, DP-SGD training pipeline, synthetic data generation, and benchmark evaluation framework. They led the project and completed the end-to-end implementation and report writing.

\medskip
\noindent Mehak Kapur. They ran and validated DuckDB queries to ingest the Parquet datasets. They helped construct the unified wide training table. They implemented and executed the DP-SGD VAE pipeline to generate synthetic data. They produced evaluation outputs and visualizations comparing real versus synthetic distributions. They contributed to report writing.

\medskip
\noindent Hana Tjendrawasi. They ran and validated DuckDB queries to ingest the Parquet datasets. They helped construct the unified wide training table by aligning schemas across sources. They supported the DP-SGD workflow through preprocessing and experiment setup. They assisted with reviewing evaluation metrics and results. They contributed to report writing.

\medskip
\noindent Phuc Tran. They performed exploratory data analysis on raw telemetry tables. They conducted schema verification and column mapping between raw and reporting tables.
