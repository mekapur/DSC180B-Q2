...
Website: \url{https://mekapur.github.io/DSC180B-Q2/}
\end{center}
...
Wall-clock costs vary substantially: wide-table DP-VAE requires 360 minutes (CPU), per-table synthesis completes in $\sim$90 minutes, MST runs in under 10 minutes per table, and PE variants finish in 37--52 minutes but incur API costs (under \$5 USD per run).

\subsubsection{Overall interpretation}

Foundation models import their own distributional priors rather than learning the target distribution. Conditional PE improves over vanilla PE (3/10 vs.\ 1/10 passed) by guiding generation with DP-noised aggregates, but gains concentrate on queries aligned with the stratification. The distributional mismatch between the model's prior and the real data reduces the effective candidate pool, consistent with \citeauthor{swanberg2025apiaccessllmsuseful}'s finding that LLMs capture 1-way marginals reasonably but are inaccurate on $k$-way marginals. Promising mitigations include two-stage zero/nonzero models, relational DP synthesis that handles multi-table structure directly, and per-query-type method ensembling.
...
Under binary pass-rate scoring, per-table DP-SGD and MST each pass 6 of 21 queries (28.6\%), the wide-table DP-VAE passes 1 of 10 (10.0\%), and conditional PE passes 3 of 10 (30.0\%). The normalized error metric (Section~\ref{sec:normalized-error}) provides further differentiation. Per-table DP-SGD achieves a median normalized error of 2.93$\times$, followed by conditional PE at 3.79$\times$, MST at 10.76$\times$, and wide-table DP-VAE at 15.34$\times$. Per-table DP-SGD achieves errors below 5$\times$ on half the benchmark, while conditional PE achieves the lowest error on 4 of its 9 evaluated queries. MST excels on ranking queries but suffers extreme errors on multi-table joins.

Two structural findings emerge. First, wide-table sparsity (93--99.9\% zeros), not the choice of algorithm, is the dominant failure mode. Second, per-table independence destroys cross-table correlations, while the wide-table approaches at least preserve some relational structure. Conditional PE partially addresses sparsity through per-stratum prompting.

Our PE results are consistent with \citet{swanberg2025apiaccessllmsuseful}. Vanilla PE underperforms training-based methods, but conditional PE closes much of the gap on queries aligned with the stratification. The broader lesson is that data representation and workload alignment matter more than model class alone. The most promising next step is relational DP synthesis that models linked tables directly, handling sparsity and cross-table dependence in a single mechanism.

\subsection{Acknowledgements}

We thank our advisors Yu-Xiang Wang and Bijan Arbab for their guidance and support throughout this project. We also thank the Scalable Statistical Machine Learning (S2ML) Lab at UC San Diego for providing OpenAI API access used in the Private Evolution experiments, and Bijan Arbab for facilitating access to the Intel DCA telemetry corpus through the HDSI Industry Data Repository and for compiling the analytical SQL benchmark queries used in our evaluation.
