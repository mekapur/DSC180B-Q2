...
To isolate the sparsity artifact of the wide-table approach, we implement an independent per-table synthesis baseline. Each of the 19 reporting tables is synthesized separately using DP histogram mechanisms calibrated to $\varepsilon = 4.0$ per table. For each table, we aggregate to the \texttt{guid} level, compute join rates (fraction of anchor \texttt{guid}s with data), and generate proportionally sized synthetic tables.

This approach sacrifices cross-table correlations by design. Synthetic \texttt{guid}s in one table bear no relationship to those in another. However, it avoids the zero-inflation problem: each table's model trains only on \texttt{guid}s that have data, so the input distribution is not dominated by zeros. Per-table synthesis serves as a diagnostic: if it outperforms the wide-table approach on single-table queries while failing on multi-table joins, that confirms the sparsity hypothesis.

\subsection{MST baseline}

As a non-neural baseline, we implement the Maximum Spanning Tree (MST) algorithm for synthetic data generation. MST selects a set of low-dimensional marginals (pairwise column distributions) using the exponential mechanism, estimates them with Gaussian noise, and generates synthetic records consistent with the noisy marginals via probabilistic graphical models. This marginal-based approach represents the current state of the art for tabular DP synthesis and provides the reference point against which both DP-SGD and PE should be measured.
...
\subsection{Three-method comparison}

Table~\ref{tab:method-comparison} summarizes the benchmark performance of the three completed synthesis methods. Both per-table methods (DP histogram and MST) pass 6 of 21 queries; the wide-table DP-VAE passes 1 of 8 evaluated queries.
...
\bottomrule
\end{tabular}
\caption{Benchmark performance across synthesis methods. ``Passed'' counts queries with score $\geq 0.5$. The wide-table method evaluates only 8 queries because 13 require per-row columns absent from the wide-table representation.}
\label{tab:method-comparison}
\end{table}

The methods pass different subsets of queries. All three correctly identify the most popular browser per country. Both per-table methods pass the battery geographic summary and display vendor percentage queries. MST additionally passes the browser usage distribution, while per-table DP-SGD additionally passes the on/off sleep summary and RAM utilization histogram. Neither per-table method passes any of the three userwait queries or the display connection type query.

Figure~\ref{fig:method-comparison} visualizes the overall pass rates and average scores.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/method_comparison_overall.pdf}
...
Figure~\ref{fig:method-heatmap} shows per-query scores across all three methods. The heatmap reveals that no method achieves high scores on aggregate queries involving continuous metrics (the top rows), while categorical and ranking queries (bottom rows) show more variation across methods.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{figures/method_comparison_heatmap.pdf}
\caption{Per-query scores across methods. Green indicates high fidelity; red indicates large discrepancy. ``N/A'' marks queries not evaluated for the wide-table method.}
\label{fig:method-heatmap}
\end{figure}

Figure~\ref{fig:method-by-type} breaks down average scores by query type. All three methods achieve their highest scores on distribution queries (browser percentage, vendor percentage). The per-table methods outperform the wide-table approach on histogram queries (RAM utilization) because the wide-table sparsity problem is absent. Aggregate queries involving multi-table joins remain the hardest category for all methods, with average scores below 0.3.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/method_comparison_by_type.pdf}
\caption{Average query score by query type across synthesis methods. Distribution and ranking queries achieve higher scores than aggregate queries requiring cross-table joins.}
\label{fig:method-by-type}
\end{figure}

\subsection{Private Evolution: preliminary observations}
...
Research question (1), which method achieves higher benchmark scores, has a nuanced answer. The per-table DP-SGD and MST baseline both pass 6 of 21 queries (28.6\%), with MST achieving a slightly higher average score (0.328 vs.\ 0.303). The wide-table DP-VAE passes only 1 of 8 evaluated queries (12.5\%). No method comes close to passing all queries. The methods pass different subsets: MST excels on browser distribution queries (popular\_browsers\_by\_count passes with TV $\leq 0.15$), while per-table DP-SGD excels on time-segmented queries (on/off sleep summary, RAM histogram). This complementarity suggests that no single synthesis approach dominates across all query types.

Research question (2), whether error compounds across multi-table joins, receives a clear affirmative. The wide-table design preserves joint distributions in principle but creates a feature matrix where most columns are nearly degenerate. The VAE collapses these sparse columns to their mode (zero), destroying any signal in the nonzero tail. The per-table approach eliminates the zero-inflation problem for single-table queries but loses cross-table correlations entirely. Both per-table methods achieve comparable results on the 21-query benchmark, indicating that the wide-table sparsity artifact is the dominant failure mode rather than the choice of synthesis algorithm.

The browser ranking result demonstrates that synthesis succeeds for well-populated categorical attributes. The joint distribution between country and browser preference is dense (every \texttt{guid} has a country; most have web browsing data), so all three methods learn meaningful conditional distributions. Both per-table approaches achieve over 90\% country accuracy on this query.
...
\begin{itemize}
    ...
    \item Restricting the wide table to \texttt{guid}s with sufficient coverage (e.g., present in $\geq 3$ metric tables), reducing sparsity at the cost of population representativeness.
...
Three methods have completed evaluation against the 21-query benchmark. The per-table methods (DP-SGD and MST) each pass 6 of 21 queries at 28.6\% pass rate. The wide-table DP-VAE passes 1 of 8 evaluated queries (12.5\%). No method achieves satisfactory fidelity on queries involving sparse continuous metrics such as power consumption, CPU frequency, or temperature, where the underlying reporting tables cover fewer than 1\% of \texttt{guid}s.

The central finding is that wide-table sparsity, not the choice of synthesis algorithm, is the dominant failure mode. The wide-table approach preserves joint distributions in principle but creates a feature matrix where 93--99.9\% of values are zero. Both per-table methods avoid this problem and achieve comparable results despite using different algorithms (neural network vs.\ graphical model), confirming that the bottleneck is data structure rather than model capacity.

Preliminary PE results suggest that foundation models import their own distributional priors rather than learning the target distribution, generating systematic biases in categorical marginals and hallucinating values outside the real vocabulary. Full PE evaluation is pending completion of the generation run.

These results identify two directions for future work. First, relational DP synthesis methods that handle multi-table structure directly (without flattening to a wide table) could eliminate the zero-inflation problem while preserving cross-table correlations. Second, post-processing techniques such as mode patching and constraint enforcement could improve fidelity on categorical distributions where the underlying signal is present but noisy.
