...
At the time of writing, PE generation via the OpenAI Batch API is ongoing (11 of 24 chunks completed, producing 86{,}865 of a target 150{,}000 synthetic records). Preliminary analysis of the completed chunks reveals several patterns.
...
One notable exception: display device columns. The PE data shows 87.5\% nonzero rate for \texttt{disp\_num\_displays} (vs.\ 20.9\% real), suggesting the model over-generates display data. Browser duration columns show a similar pattern: 13.3\% nonzero rate for chrome (vs.\ 5.3\% real).
...
The preliminary PE results suggest that foundation models are not well-suited to this particular domain. Rather than learning the DCA-specific distribution, the model imports its own priors (e.g., over-representing Win11, a more recent OS that dominates the model's training data rather than the DCA corpus vintage). The hallucinated categorical values further reduce effective sample size by generating records that cannot match any real data point in the nearest-neighbor histogram. This aligns with \citeauthor{swanberg2025apiaccessllmsuseful}'s finding that LLMs capture 1-way marginals reasonably but are inaccurate on $k$-way marginals.

The per-table DP synthesis approach (notebook 08, in progress) addresses the zero-inflation problem directly by training each table's model only on \texttt{guid}s that have data. This eliminates the degenerate distribution but introduces a different failure mode: cross-table correlations are lost entirely. The tradeoff between these two failure modes is the central diagnostic of our evaluation.

Several additional mitigation strategies warrant investigation:
...
We presented an evaluation framework for differentially private synthetic data generation on Intel's DCA telemetry corpus, implementing three synthesis approaches: a wide-table DP-VAE trained with DP-SGD, per-table DP histogram synthesis, and Private Evolution via foundation model APIs.

The wide-table DP-VAE ($\varepsilon = 3.996$, $\delta = 10^{-5}$) preserves categorical joint distributions (42/50 countries receive the correct top browser) but fails on all continuous metrics due to extreme zero-inflation. This result identifies the sparsity structure of multi-table joins as a critical factor in the success or failure of DP synthetic data generation. When coverage varies across tables (as it does in any real multi-table dataset), a unified representation concentrates nearly all probability mass at zero for sparse columns, and standard generative models learn to reproduce this mode.

Preliminary PE results suggest that foundation models import their own distributional priors rather than learning the target distribution, generating systematic biases in categorical marginals and hallucinating values outside the real vocabulary. The sparsity problem persists: the model generates near-zero values for sparse numeric columns at rates comparable to, or worse than, the DP-VAE.

The complete comparison across all four methods (wide-table DP-SGD, per-table DP-SGD, Private Evolution, and MST baseline), using matched privacy budgets and the same 21-query benchmark, is ongoing. Per-table DP-SGD and MST results will provide the diagnostic baselines needed to disentangle the effect of cross-table correlation loss from the effect of zero-inflation.
