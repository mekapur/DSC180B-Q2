...
Two tables receive dedicated DP-VAE treatment. The sysinfo anchor table (1{,}000{,}000 rows, 9 categorical and 1 numeric column) uses a VAE with latent dimension 32, hidden size 256, batch size 4{,}096, 15 epochs, KL weight 0.1, trained with Opacus at $\varepsilon = 4.0$, $\delta = 10^{-5}$. Categoricals are top-50 binned and one-hot encoded. The cpu\_metadata table (1{,}000{,}000 rows, 5 categorical and 1 numeric column) uses the same architecture.
...
The methods pass different subsets of queries. Table~\ref{tab:per-query-scores} reports per-query scores. All three correctly identify the most popular browser per country. Both per-table methods pass the battery geographic summary and display vendor percentage queries. MST additionally passes the browser usage distribution and two application ranking queries, while per-table DP-SGD additionally passes the on/off sleep summary and RAM utilization histogram. Neither per-table method passes any of the three userwait queries or the display connection type query.

\begin{footnotesize}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.40\linewidth}>{\raggedright\arraybackslash}p{0.12\linewidth}ccc}
\caption{Per-query scores for all three synthesis methods. Scores marked with $\checkmark$ pass the 0.5 threshold. ``---'' indicates queries not evaluable for the wide-table method (they require per-row columns absent from the wide-table representation).}
...
\bottomrule
\endlastfoot
\texttt{avg\_platform\_power\_c0\_freq\_temp\_by\_chassis} & Agg+Join & 0.167 & 0.000 & 0.000 \\
\texttt{battery\_power\_on\_geographic\_summary} & Geo/Demo & 0.250 & 1.000\,$\checkmark$ & 0.500\,$\checkmark$ \\
\texttt{Xeon\_network\_consumption} & Geo/Demo & 0.250 & 0.250 & 0.250 \\
\texttt{persona\_web\_cat\_usage\_analysis} & Pivot & 0.065 & 0.419 & 0.194 \\
\texttt{pkg\_power\_by\_country} & Geo/Demo & 0.333 & 0.333 & 0.333 \\
\texttt{most\_popular\_browser\ldots country} & Top-$k$ & 1.000\,$\checkmark$ & 1.000\,$\checkmark$ & 1.000\,$\checkmark$ \\
\texttt{popular\_browsers\ldots percentage} & Histogram & 0.000 & 0.333 & 0.667\,$\checkmark$ \\
\texttt{ram\_utilization\_histogram} & Histogram & 0.000 & 0.500\,$\checkmark$ & 0.000 \\
\midrule
\texttt{battery\_on\_duration\_cpu\_family\_gen} & Geo/Demo & --- & 0.000 & 0.000 \\
\texttt{display\_devices\ldots durations\_ac\_dc} & Agg+Join & --- & 0.000 & 0.000 \\
\texttt{display\_devices\_vendors\_percentage} & Agg+Join & --- & 1.000\,$\checkmark$ & 1.000\,$\checkmark$ \\
\texttt{mods\_blockers\_by\_osname\ldots} & Agg+Join & --- & 0.250 & 0.250 \\
\texttt{on\_off\_mods\_sleep\_summary\ldots} & Pivot & --- & 0.636\,$\checkmark$ & 0.273 \\
\texttt{server\_exploration\_1} & Row-level & --- & 0.143 & 0.429 \\
\texttt{top\_mods\_blocker\_types\ldots} & Agg+Join & --- & 0.000 & 0.000 \\
\texttt{top\_10\_apps\ldots focal\_time} & Top-$k$ & --- & 0.000 & 0.000 \\
\texttt{top\_10\_apps\ldots system\_count} & Top-$k$ & --- & 0.000 & 1.000\,$\checkmark$ \\
\texttt{top\_10\_apps\ldots total\_detections} & Top-$k$ & --- & 0.500\,$\checkmark$ & 1.000\,$\checkmark$ \\
\texttt{userwait\_top\_10\_wait\_processes} & Top-$k$ & --- & 0.000 & 0.000 \\
\texttt{userwait\_top\_10\ldots wait\_type\_ac\_dc} & Top-$k$ & --- & 0.000 & 0.000 \\
\texttt{userwait\_top\_20\ldots ac\_dc\_unknown} & Top-$k$ & --- & 0.000 & 0.000 \\
\end{longtable}
\end{footnotesize}
...
\caption{Pass rate sensitivity to evaluation thresholds. Left: relative error threshold (TV = 0.15, $\rho$ = 0.5 fixed). Center: total variation threshold (RE = 0.25, $\rho$ = 0.5 fixed). Right: Spearman $\rho$ threshold (RE = 0.25, TV = 0.15 fixed). Dashed gray lines indicate default thresholds used throughout.}
...
Three methods have completed evaluation against the 21-query benchmark. Per-table DP-SGD and MST each pass 6 of 21 queries (28.6\% pass rate), passing different subsets that reflect their algorithmic strengths: DP histogram synthesis preserves continuous distributions (on-time RE = 0.13) while MST preserves rankings (Spearman $\rho$ = 1.00 on application counts). The wide-table DP-VAE passes 1 of 8 evaluated queries (12.5\%), with all continuous metrics near zero due to extreme sparsity in the wide-table representation.
