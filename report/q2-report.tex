...
The query score is the fraction of metric columns that pass. A query is marked as passing when its score is at least 0.5, with query-specific overlap thresholds defined in the evaluation metadata. We also test threshold sensitivity in Section~\ref{fig:sensitivity} to verify that comparative conclusions are not driven by a single cutoff choice.
...
A follow-up iteration addressed the root cause of the 2/10 result: the per-query noise $\sigma \approx 150$ overwhelmed all but the largest strata. Three changes were applied simultaneously:
...
avg\_platform\_power\_c0\_freq\_temp & 0.000 & 0.000 & 0.000 & 0.000 \\
