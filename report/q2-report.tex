...
The approach proceeds in four phases. First, we compute aggregate statistics per stratum from the real data, where each stratum corresponds to a unique combination of categorical values and active numeric groups. Second, we add calibrated Gaussian noise to every aggregate query via the Gaussian mechanism, spending a privacy budget of $\varepsilon_{\text{agg}} = 3.5$. We employ zero-concentrated differential privacy (zCDP) composition \citep{bun2016concentrated}, converting the total $(\varepsilon_{\text{agg}}, \delta)$ budget to a zCDP parameter $\rho_{\text{total}}$ and splitting $\rho$ equally across 7 query groups (battery, memory, on/off, browser/webcat, hardware, network, and hints). This yields per-query noise scales $\sigma_g = \sqrt{k_g / (2\rho_g)}$ ranging from 24.6 (network, $k=40$ queries) to 68.2 (browser/webcat, $k=308$ queries), substantially tighter than the $\sigma \approx 150$ that naive advanced composition would require across all 1{,}056 queries jointly. Third, we use the noisy aggregates to construct per-stratum prompts that specify target counts and averages for each numeric group. Since these prompts are deterministic functions of the DP-released aggregates, this step is free post-processing under DP. Fourth, we generate a $2\times$ candidate pool across all strata and select $N_{\text{synth}} = 5{,}000$ records via the DP nearest-neighbor histogram mechanism, spending $\varepsilon_{\text{hist}} = 0.5$.
...
Per-table DP-SGD achieves the best median normalized error (2.72), followed by MST (10.96) and wide-table DP-VAE (12.88). This ranking differs from the binary pass-rate ranking, where per-table and MST were tied at 28.6\%. The normalized metric reveals that per-table DP-SGD's errors are consistently smaller relative to natural variation, while MST's high mean (1{,}294) is driven by extreme outliers on aggregate queries that involve multi-table joins (e.g., \texttt{SleepPivot} at 8{,}655$\times$ and \texttt{DisplayAgg} at 3{,}850$\times$). Normalized errors for PE are omitted because the baseline split evaluation was not rerun for the current PE experiments. Metrics where the baseline variation $|\Delta_j(D_1,D_2)|$ is below $10^{-8}$ (i.e., the two random halves produce identical values) are excluded, since they offer no natural variation to normalize against.

Table~\ref{tab:normalized-error-per-query} provides per-query normalized errors for the three training-based methods.

\begin{footnotesize}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.30\linewidth}>{\raggedright\arraybackslash}p{0.11\linewidth}*{3}{>{\centering\arraybackslash}p{0.14\linewidth}}}
...
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-table} & \textbf{MST} \\
\midrule
\endfirsthead
\toprule
\textbf{Query} & \textbf{Type} & \textbf{Wide} & \textbf{Per-table} & \textbf{MST} \\
...
\bottomrule
\endlastfoot
\texttt{XeonGeo} & Geo & 96.5 & \textbf{6.5} & 593.4 \\
\texttt{ChassisAgg} & Agg & 309.3 & \textbf{4.0} & \textbf{4.0} \\
\texttt{BatteryDemo} & Geo & --- & \textbf{12.6} & 248.1 \\
\texttt{BatteryGeo} & Geo & 139.1 & \textbf{2.5} & 320.5 \\
\texttt{DisplayAgg} & Agg & --- & \textbf{13.7} & 3{,}850.4 \\
\texttt{VendorAgg} & Agg & --- & 44.6 & \textbf{27.5} \\
\texttt{BlockerAgg} & Agg & --- & \textbf{16.1} & 35.3 \\
\texttt{BrowserRank} & Top-$k$ & 9.0 & 4.0 & \textbf{3.0} \\
\texttt{SleepPivot} & Pivot & --- & \textbf{19.7} & 8{,}655.3 \\
\texttt{PersonaPivot} & Pivot & 65.4 & \textbf{3.4} & 35.3 \\
\texttt{PowerGeo} & Geo & 616.6 & 3.3 & \textbf{1.0} \\
\texttt{BrowserHist} & Hist & 95.5 & 118.0 & \textbf{25.5} \\
\texttt{RamHist} & Hist & 29.8 & \textbf{38.3} & 110.9 \\
...
\end{tikzpicture}
\end{minipage}
\caption{Pass rate sensitivity to evaluation thresholds for the three training-based methods. Left: relative error threshold (TV = 0.15, $\rho$ = 0.5 fixed). Center: total variation threshold (RE = 0.25, $\rho$ = 0.5 fixed). Right: Spearman $\rho$ threshold (RE = 0.25, TV = 0.15 fixed). Dashed gray lines indicate the default thresholds. PE is omitted because the sensitivity sweep was not rerun for the current PE experiments.}
