...
Table~\ref{tab:method-comparison} summarizes the benchmark performance of all four synthesis methods. Both per-table methods (DP histogram and MST) pass 6 of 21 queries; the wide-table DP-VAE passes 1 of 8 evaluated queries. Private Evolution, which evaluates the same 8 wide-table queries, passes TBD.
...
Private Evolution & 8 & TBD & TBD & TBD & TBD \\
\bottomrule
\end{tabular}
...
\bottomrule
\endlastfoot
\texttt{avg\_platform\_power\ldots chassis} & Agg+Join & \cellcolor{scorelo}0.167 & \cellcolor{scorelo}0.000 & \cellcolor{scorelo}0.000 & \cellcolor{scorena}TBD \\
\texttt{battery\_power\_on\ldots summary} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}0.500\,$\checkmark$ & \cellcolor{scorena}TBD \\
\texttt{Xeon\_network\_consumption} & Geo/Demo & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scoremid}0.250 & \cellcolor{scorena}TBD \\
\texttt{persona\_web\_cat\_usage\ldots} & Pivot & \cellcolor{scorelo}0.065 & \cellcolor{scoremid}0.419 & \cellcolor{scorelo}0.194 & \cellcolor{scorena}TBD \\
\texttt{pkg\_power\_by\_country} & Geo/Demo & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & \cellcolor{scoremid}0.333 & \cellcolor{scorena}TBD \\
\texttt{most\_popular\_browser\ldots country} & Top-$k$ & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorehi}1.000\,$\checkmark$ & \cellcolor{scorena}TBD \\
\texttt{popular\_browsers\ldots percentage} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scoremid}0.333 & \cellcolor{scorehi}0.667\,$\checkmark$ & \cellcolor{scorena}TBD \\
\texttt{ram\_utilization\_histogram} & Histogram & \cellcolor{scorelo}0.000 & \cellcolor{scorehi}0.500\,$\checkmark$ & \cellcolor{scorelo}0.000 & \cellcolor{scorena}TBD \\
\midrule
...
\addplot[fill=RoyalBlue!50, draw=RoyalBlue] coordinates {(Wide,1) (Per-table,6) (MST,6) (PE,0)}; % PE TBD
\end{axis}
\end{tikzpicture}
\end{minipage}
\hfill
...
\addplot[fill=RoyalBlue!50, draw=RoyalBlue] coordinates {(Wide,0.258) (Per-table,0.303) (MST,0.328) (PE,0)}; % PE TBD
\end{axis}
\end{tikzpicture}
\end{minipage}
...
\addplot[fill=Goldenrod!60, draw=Goldenrod] coordinates {% PE TBD
    (Agg+Join,0) (Top-$k$,0) (Geo/Demo,0) (Histogram,0) (Pivot,0)
};
...
\addplot[Goldenrod, mark=diamond*] coordinates {% PE TBD
    (0.05,0) (0.10,0) (0.15,0) (0.20,0)
    (0.25,0) (0.50,0) (0.75,0) (1.00,0)
};
